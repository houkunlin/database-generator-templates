...
${gen.setFilename("SearchForm.vue")}
${gen.setFilepath("ui/${entity.name}/")}
<template>
  <!-- 查询表单信息 -->
  <div class="table-page-search-wrapper">
    <a-form :label-col="labelCol" :wrapper-col="wrapperCol" @submit="onSearch">
      <a-row :gutter="10">
        <#list fields as field>
            ...
            <a-col v-bind="queryCol">
              <a-form-item label="${field.comment}"><#if field.column.comment?trim?length gt 0 && field.comment != field.column.comment><!-- 数据库字段说明：${field.column.comment} --></#if>
                ...
                <#elseif ["BIT","BOOLEAN"]?seq_contains(type)>
                  <a-select v-model="queryParam.${field.name}" placeholder="请选择${field.comment}" default-value="" allowClear >
          ...
          <a-col v-bind="btnCol">
              <a-form-item :label-col="{sm: {span: 0}}">
                  <a-button type="primary" @click="onSearch" :loading="loading">查询</a-button>
                  <a-button style="margin-left: 8px" @click="onReset">重置</a-button>
                  <a v-if="showAdvanced" style="margin-left: 8px" @click="advanced = !advanced">
                      {{ advanced ? '收起' : '展开' }}
  ...
  data () {
    return {
      ...
      wrapperCol: { sm: { span: 18 }, md: { span: 16 }, lg: { span: 14 }, xl: { span: 16 }, xxl: { span: 18 } },
        btnCol: { sm: { span: 24 }, md: { span: 12 }, lg: { span: 8 }, xl: { span: 6 }, xxl: { span: 4 } },
        queryCol: { sm: { span: 24 }, md: { span: 12 }, lg: { span: 8 }, xl: { span: 8 }, xxl: { span: 6 } }
    }
  },
  watch: {
    value (val) {
